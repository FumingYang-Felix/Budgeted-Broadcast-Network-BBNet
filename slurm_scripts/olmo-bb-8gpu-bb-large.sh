#!/bin/bash
#===============================================================================
# OLMo BB Pruning - 8 GPU (2 nodes × 4 GPUs) - LARGE SCALE
# batch=64/GPU (global batch=512), seq=4096, 500 steps
...
#SBATCH -o slurm_out/bb-8gpu-large-%j.out
#SBATCH -e slurm_out/bb-8gpu-large-%j.err
#SBATCH --job-name=olmo-bb-large
...
MAX_STEPS=500
BATCH_SIZE=64                        # Per GPU batch size (global batch = 64 × 8 = 512)
SEQ_LEN=4096
...
echo ""
echo "==============================================================================="
echo "        BB PRUNING - 8 GPU LARGE SCALE (2.1B tokens)                          "
echo "==============================================================================="
echo ""
...
echo "=== Starting BB Training (8 GPUs, 2.1B tokens) ==="
START_TIME=$(date +%s)

CKPT_DIR="checkpoints/olmo-bb-8gpu-large-$SLURM_JOB_ID"
...
srun python -m torch.distributed.run \
    ...
    --save_report slurm_out/report-bb-8gpu-large-$SLURM_JOB_ID.json \
...
if [ -f "slurm_out/report-bb-8gpu-large-$SLURM_JOB_ID.json" ]; then
    echo "=== Training Report ==="
    cat slurm_out/report-bb-8gpu-large-$SLURM_JOB_ID.json
fi
