#!/bin/bash
#===============================================================================
# OLMo-1B Mid-Training with Dolma Dataset - Dense Baseline
# Using official OLMo configuration:
#   - Sequence length: 2048
#   - Global batch size: ~4M tokens (2048 instances × 2048 tokens)
...
#SBATCH -o slurm_out/dolma-dense-8gpu-%j.out
#SBATCH -e slurm_out/dolma-dense-8gpu-%j.err
#SBATCH --job-name=olmo-dolma-dense

#===============================================================================
# OLMo Official Configuration
#===============================================================================

# OLMo-1B official settings:
# - Sequence length: 2048
# - Global batch size: 2048 instances = 4M tokens
# - Per GPU: 2048 / 8 GPUs = 256 instances
# - But 256 may OOM, so we use gradient accumulation

MAX_STEPS=100
# OLMo official config from yaml:
# global_train_batch_size: 512
# device_train_microbatch_size: 4
# With 8 GPUs: 512 / 8 = 64 effective per GPU
# So: micro_batch=4, grad_accum=16 -> 4*16=64

BATCH_SIZE=4                         # OLMo official micro-batch
GRAD_ACCUM=16                        # To achieve 64 effective per GPU
# Effective batch per GPU: 4 * 16 = 64
# Total effective batch: 64 * 8 = 512 (OLMo official)
# Tokens per step: 512 * 2048 = 1M tokens

SEQ_LEN=2048                         # OLMo official sequence length
LR=3e-5                              # OLMo official mid-training LR (not 3e-4!)
LOG_INTERVAL=10
...
echo ""
echo "==============================================================================="
echo "        OLMo-1B Mid-Training with Dolma - DENSE BASELINE                      "
echo "==============================================================================="
echo ""
...
CKPT_DIR="checkpoints/olmo-dolma-dense-$SLURM_JOB_ID"

echo "=== Starting OLMo Mid-Training with Dolma (Dense) ==="
START_TIME=$(date +%s)
...
srun python -m torch.distributed.run \
    ...
    --use_dolma \
    --save_report slurm_out/report-dolma-dense-$SLURM_JOB_ID.json \
